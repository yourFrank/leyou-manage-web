    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="downGood(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="upGood(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      downGood(id){
        this.$http.put("/item/spu/downgood/" + id)
          .then(() => {
      ...
      upGood(id){
        this.$http.put("/item/spu/upgood/" + id)
          .then(() => {
